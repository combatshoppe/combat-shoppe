...
/* exported ParserModule */
var ParserModule = angular.module('ParserModule', ['DataModule']); // eslint-disable-line no-unused-vars
...
class Parser {
	...
	_monsterJsonToSchema(json){
		...
		const creature = new CreatureSchema( {cr:cr,name:name,int:int,cha:cha,dex:dex,str:str,con:con,wis:wis,speed:speed,ac:ac,pb:pb,hp:hp,actions:creatureActions,src:src,size:intSize}); // eslint-disable-line no-undef
	...
	parseActionFinal(arrayLength, actionArray, attributesArray, type){
		...
		for (var q = 0; q < arrayLength; q++) {
			...
			const action =  new ActionSchema( {name: parsedName, toHitBonus: attributes[0], range: attributes[1], primaryDamage: attributes[2], primaryDice: attributes[3]+'d'+attributes[4]+'+'+attributes[5] }); // eslint-disable-line no-undef
